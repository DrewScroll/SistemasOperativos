...
public:
	...
	AtomicLogger(Stream& stream) : m_mutex(), m_stream(stream)
	{
	}
...
public:
	...
	Philosopher(const std::string& name, ForkPtr fork_left, ForkPtr fork_right, AtomicLoggerOstreamPtr p_logger):
	m_name(name), m_continue(true), mp_fork_left (fork_left), mp_fork_right (fork_right),
	m_thread(boost::thread(boost::bind(&Philosopher::thread_func, this, &m_continue, mp_fork_left, mp_fork_right))),
	m_meals_left(NUM_MEALS), mp_logger(p_logger)
	{
	}
	~Philosopher()
	{
...
private:
	...
	void thread_func(volatile bool* p_continue, ForkPtr fork_left, ForkPtr fork_right) {
	 ...
	 while(p_continue && m_meals_left){
	  mp_logger->log(boost::str(boost::format("%1% esta pensando") % this->m_name));
	  wait();
	  mp_logger->log(boost::str(boost::format("%1% tiene hambre") % this->m_name));
	  ...
	  if(can_grab_fork(fork_left)){
	   ...
	   if(can_grab_fork(fork_right)){
	    ...
	    mp_logger->log(boost::str(boost::format("%1% esta comiendo (%2%)...") % m_name % m_meals_left));
	    wait();
	    --m_meals_left;
	   } else {
	  ...
	  if (failed_to_grab_fork){
	   mp_logger->log(boost::str(boost::format("%1% no pudo agarrar los tenedores; esperando...") % m_name));
	   failed_to_grab_fork = false;
	   wait();
	  }
	 }
		mp_logger->log(boost::str(boost::format("%1% termino de comer") % m_name));
	}
...
int main(){
	...
	forks.reserve(N);
	for(int i= 0; i<N; ++i) {
	 philosophers.push_back(PhilosopherPtr( new Philosopher(names[i],forks[i], forks[(i+1)%N], p_logger)));
	}
	...
	p_logger->log("Todos terminaron de comer");
